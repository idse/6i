    ...
    methods
        ...
        function this = Metadata(dataDir, manualMeta)
            ...
            if nargin == 1 || nargin == 2
                ...
                [C, matches] = strsplit(filename,'_[Ffwptm][0-9]+','DelimiterType','RegularExpression');
                if ~isempty(matches)
                    ndigits = numel(matches{1}) - 2;
                    matchformat =  [matches{1}(1:2) '%.' num2str(ndigits) 'd'];
                    this.filenameFormat = [C{1} matchformat C{2}];
                else
