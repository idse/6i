function [rowsol,cost,v,u,costMat] = lapjv_v2(costMat,resolution)
...
if any(isnan(costMat),'all')
    costMat(isnan(costMat))=Inf;
end

if issparse(costMat)
    costs = nonzeros(costMat);
    maxcost = max(max(costs)*dim+1,1);
else
    maxcost=max(costMat(~isinf(costMat)))*dim+1;
end
...
% column reduction
%disp('Initilization phase: column reduction')
...
%disp('Reduction transfer from unassigned to assigned rows')
for i=1:dim
...
%disp('Augmenting reduction of unassigned rows')
loopcnt = 0;
while loopcnt < 2
    loopcnt = loopcnt + 1;
    % scan all free rows
...
%disp('Augmentation phase: augment solution for each free rows')
for f=1:numfree
